...
const PurchaseReturnForm = ({ isOpen, onClose, onSave, editData = null, viewOnly = false }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (editData) {
                ...
                const dataToSet = {
                    ...getInitialFormData(),
                    invoiceNumber: generateUniqueId("PRN"),
                    invoiceDate: new Date().toISOString().split('T')[0]
                };
                setFormData(dataToSet);
                setStep(1);
        ...
        if (editData && formData.returnAgainst && allInvoices.length > 0) {
            ...
            if (selectedInv) {
                ...
                const updatedItems = formData.items.map(item => {
                    // If max_quantity is already set from invoice and doesn't equal default large number
    ...
    const getOriginalReturnedQty = (itemId, sourceType) => {
        if (!editData || !Array.isArray(editData.purchase_item)) return 0;
        const matchedOriginalItem = editData.purchase_item.find(oi => {
            let ost = oi.source_type || "item";
            if (ost === "item") ost = "customized_product";
            if (ost !== sourceType) return false;
            const oiId = ost === "raw_material"
                ? (oi.source_id || "")
                : (oi.items?.id || oi.items || oi.source_id || "");
            return String(oiId) === String(itemId);
        });
        return matchedOriginalItem ? (parseFloat(matchedOriginalItem.quantity) || 0) : 0;
    };

    const getItemCurrentInventoryStock = (itemId, sourceType) => {
        if (productsList.length === 0 && rawMaterialsList.length === 0 && customizedProductsList.length === 0) {
            return 999999;
        }
        let invItem = null;
        if (sourceType === "raw_material") {
            invItem = rawMaterialsList.find(r => String(r.id) === String(itemId));
            return invItem ? (parseFloat(invItem.quantity || invItem.current_quantity) || 0) : 0;
        } else if (sourceType === "customized_product") {
            invItem = customizedProductsList.find(c => String(c.id) === String(itemId));
            return invItem ? (parseFloat(invItem.current_quantity || invItem.quantity) || 0) : 0;
        } else {
            invItem = productsList.find(p => String(p.id) === String(itemId));
            return invItem ? (parseFloat(invItem.current_quantity || invItem.quantity) || 0) : 0;
        }
    };
    const hasStockErrors = useMemo(() => {
        return formData.items.some(item => {
            ...
            const invoiceLimit = item.max_quantity !== undefined ? item.max_quantity : 999999;
            if (item.source_type === "service") {
                return qty > invoiceLimit;
            }
            const itemStock = getItemCurrentInventoryStock(item.item_id, item.source_type);
            const origReturned = getOriginalReturnedQty(item.item_id, item.source_type);
            const maxStockLimit = itemStock + (editData ? origReturned : 0);
            const allowedMax = Math.min(invoiceLimit, maxStockLimit);
            return qty > allowedMax;
        });
    }, [formData.items, productsList, rawMaterialsList, customizedProductsList, editData]);
    ...
    const handleSelectInvoiceToReturn = async (invoiceId) => {
        ...
        const supplierId = supplier?.id || (typeof selectedInv.supplier_id !== 'object' ? selectedInv.supplier_id : "");
        ...
        if (Array.isArray(itemsList)) {
            ...
            }).filter(item => item.max_quantity > 0); // Only return items that actually have quantity remaining
        }
        ...
        if (field === "quantity") {
            if (value === "") {
                ...
                if (isNaN(qty)) {
                    ...
                    const itemStock = getItemCurrentInventoryStock(newItems[index].item_id, newItems[index].source_type);
                    const origReturned = getOriginalReturnedQty(newItems[index].item_id, newItems[index].source_type);
                    const maxStockLimit = itemStock + (editData ? origReturned : 0);
                    const invoiceLimit = newItems[index].max_quantity !== undefined ? newItems[index].max_quantity : 999999;
                    const allowedMax = newItems[index].source_type === "service" ? invoiceLimit : Math.min(invoiceLimit, maxStockLimit);

                    if (qty < 0) {
                        newItems[index].quantity = 0;
                    } else if (qty > allowedMax) {
                        let errorMsg = `Quantity for ${newItems[index].description || 'item'} cannot exceed `;
                        if (invoiceLimit < maxStockLimit) {
                            errorMsg += `invoice quantity (${invoiceLimit})`;
                        } else {
                            errorMsg += `available stock (${maxStockLimit})`;
                        }
                        dispatch(showToast({
                            message: errorMsg,
                            type: "warning"
                        }));
                        newItems[index].quantity = allowedMax;
                    } else {
    ...
    const validateForm = () => {
        if (!formData.returnAgainst) {
            dispatch(showToast({ message: "Please select an invoice to return against", type: "warning" }));
            return false;
        }
        ...
        const overLimitItem = formData.items.find(item => {
            ...
            const maxStockLimit = itemStock + (editData ? origReturned : 0);
            const invoiceLimit = item.max_quantity !== undefined ? item.max_quantity : 999999;
            const allowedMax = item.source_type === "service" ? invoiceLimit : Math.min(invoiceLimit, maxStockLimit);

            if (qty > allowedMax) return true;
            return false;
        });

        if (overLimitItem) {
            const itemStock = getItemCurrentInventoryStock(overLimitItem.item_id, overLimitItem.source_type);
            const origReturned = getOriginalReturnedQty(overLimitItem.item_id, overLimitItem.source_type);
            const maxStockLimit = itemStock + (editData ? origReturned : 0);
            const invoiceLimit = overLimitItem.max_quantity !== undefined ? overLimitItem.max_quantity : 999999;

            let errorMsg = `Quantity for ${overLimitItem.description || 'item'} cannot exceed `;
            if (invoiceLimit < maxStockLimit) {
                errorMsg += `invoice quantity (${invoiceLimit})`;
            } else {
                errorMsg += `available stock (${maxStockLimit})`;
    ...
    const handleSave = async (forcedStatus = null, isSendToClient = false, isSendOnly = false) => {
        ...
        try {
            ...
            let response;
            if (editData) {
                response = await purchaseReturnService.updateReturn(editData.id, payload);
                dispatch(showToast({ message: "Purchase Return updated successfully!", type: "success" }));
            } else {
                response = await purchaseReturnService.saveReturn(payload);
                dispatch(showToast({ message: "Purchase Return created successfully!", type: "success" }));
            }
            ...
            if (isSendToClient && newId) {
                try {
                    ...
                    await purchaseReturnService.sendReturnEmail(emailData);
            ...
            if (!error.isHandled) {
                dispatch(showToast({ message: error.message || "Error saving purchase return.", type: "error" }));
            }
        } finally {
            setIsSaving(false);
        }
                    ...
                    {!formData.returnAgainst && !editData ? (
                        <div className="py-4 px-2 max-w-4xl mx-auto font-poppins text-left">
                            ...
                            <div className="mb-6">
                                ...
                                <label className="text-xs font-semibold text-gray-700 mb-2 block">Select Purchase Invoice</label>
                                {isLoadingInvoices ? (
                                    ...
                                    <CustomSelect
                                        ...
                                        placeholder="-- Choose a Purchase Invoice of the item you want to return --"
                                    />
                                )}
                            </div>
                        ...
                        <div>
                            <div className="flex flex-wrap justify-between items-start mb-6 gap-2 font-poppins">
                                <div>
                                    <h2 className="text-xl md:text-2xl font-semibold text-gray-900">Configure Purchase Return</h2>
                            ...
                            <div className="border-t pt-6">
                                ...
                                <div className="mb-8 font-poppins">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            <label className="text-xs font-medium text-gray-700 mb-1.5 block">Supplier (Read-Only)</label>
                                            <input
                                                type="text"
                                                value={suppliers.find(s => s.id == formData.supplier)?.name || "N/A"}
                                                readOnly
                                                    ...
                                                    {formData.items.map((item, idx) => (
                                                        <tr key={idx} className="border-b border-gray-200">
                                                                        ...
                                                                        <div className="flex flex-col items-center mt-1 gap-0.5">
                                                                            {item.source_type !== "service" && (
                                                                                <span className="text-[9px] text-gray-400 block leading-tight">
                                                                                    Stock: {getItemCurrentInventoryStock(item.item_id, item.source_type)}
                                                                                </span>
                                                                            )}
                                                            ...
                                                            <td className="px-3 py-3 text-right font-semibold text-gray-900 whitespace-nowrap w-32 min-w-[120px]">
                                                                {item.source_type === "service" ? (
                                                                    <div className="flex items-center justify-end gap-1 border border-gray-200 bg-gray-50 rounded px-2 py-1.5 min-w-[120px] shadow-sm">
                                                                        <span className="text-gray-400 text-xs font-semibold">₹</span>
                                                                        <input
                                                                            type="number"
                                                                            value={item.rate}
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start mb-4 gap-4 no-print">
                                <div>
                                    <h2 className="text-xl md:text-2xl font-bold text-gray-900">
                                        {viewOnly ? "View Purchase Return" : "Preview & Send"}
                                    </h2>
                                    <p className="text-sm text-gray-500 mt-1">
                                        {isExporting ? "Processing..." : viewOnly ? "View details of your purchase return" : "Review and send your purchase return"}
                                    </p>
                                </div>
                                ...
                                <div className="bg-white rounded-lg shadow-sm overflow-x-auto md:overflow-x-visible max-w-full md:max-w-4xl mx-auto">
                                    <div className="min-w-[800px] md:min-w-full">
                                        <DocumentPreview
                                            ...
                                            filename={`PurchaseReturn-${formData.invoiceNumber}`}
                                            payload={{
                                                company: {
