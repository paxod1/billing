...
const PurchaseInvoiceForm = ({ isOpen, onClose, onSave, editData = null, viewOnly = false }) => {
    ...
    // Fetch all invoices for "Return Against"
    const fetchAllInvoices = async (returnAgainstId = null) => {
        try {
            const invoices = await purchaseInvoiceService.getAllInvoicesForDropdown(returnAgainstId);
    ...
    // Fetch all purchase orders for linking
    const fetchPurchaseOrders = async (currentOrderId = null) => {
        try {
            setIsLoadingOrders(true);
            const orders = await purchaseOrderService.getAllOrdersForDropdown(currentOrderId);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const checkEditTaxAndFetch = async () => {
                ...
                if (editData && Array.isArray(editData.purchase_item) && editData.purchase_item.length > 0) {
                    const firstItem = editData.purchase_item.find(item => {
            ...
            fetchAllInvoices(editData?.return_against);
            fetchPurchaseOrders(editData?.order_id);

            let dataToSet;
            if (editData) {
                // Populate form with editData
                const supplierData = Array.isArray(editData.supplier_id) ? editData.supplier_id[0] : editData.supplier_id;
                dataToSet = {
                    invoiceNumber: editData.invoice_number || "",
                    invoiceName: editData.invoice_name || "",
                    status: editData.status || "DRAFT",
                    supplier: supplierData?.id || (typeof editData.supplier_id !== 'object' ? editData.supplier_id : ""),
                    invoiceDate: editData.invoice_date ? new Date(editData.invoice_date).toISOString().split('T')[0] : "",
                    items: Array.isArray(editData.purchase_item) ? editData.purchase_item.map(item => {
                        let sourceType = item.source_type || "item";
                        if (sourceType === "item") {
                    ...
                    order_id: editData.order_id || null, // Preserve order_link separately
                    emailConfig: {
                ...
                dataToSet = {
                    ...getInitialFormData(),
                    invoiceNumber: generateUniqueId("PIN"),
                    invoiceDate: new Date().toISOString().split('T')[0],
                    items: [{ item_id: "", source_type: "raw_material", tax_id: "", tax_percent: 0, quantity: 1, rate: 0, amount: 0, description: "" }]
                };
            }

            // Restore state if resuming (same logic as PO)
            if (searchParams.get("action") === "resume") {
                const savedData = localStorage.getItem("pending_invoice_data");
                if (savedData) {
                    ...
                    router.replace(`/purchases/invoices?${params.toString()}`);
                }
            }
    ...
    useEffect(() => {
        if (editData && formData.returnAgainst && allInvoices.length > 0) {
            ...
            if (selectedInv) {
                const originalItems = selectedInv.purchase_item || selectedInv.order_item || [];
            ...
            if (value === "") {
                ...
                if (isNaN(qty)) {
                    ...
                    if (formData.returnAgainst && maxQty !== undefined && qty > maxQty) {
                        dispatch(showToast({
                            message: `Quantity for ${newItems[index].description || 'item'} cannot exceed original invoice quantity (${maxQty})`,
                            type: "warning"
                        }));
    ...
    const validateForm = () => {
        if (!formData.invoiceName?.trim()) {
            dispatch(showToast({ message: "Invoice Name is required", type: "warning" }));
            return false;
        }
        ...
        if (!formData.invoiceDate) {
            dispatch(showToast({ message: "Invoice Date is required", type: "warning" }));
            return false;
        }
    ...
    const handleSave = async (forcedStatus = null, isSendToClient = false, isSendOnly = false) => {
        ...
        try {
            ...
            let response;
            if (editData) {
                response = await purchaseInvoiceService.updateInvoice(editData.id, payload);
                dispatch(showToast({ message: "Purchase Invoice updated successfully!", type: "success" }));
            } else {
                response = await purchaseInvoiceService.saveInvoice(payload);
                dispatch(showToast({ message: "Purchase Invoice created successfully!", type: "success" }));
            }
            ...
            if (isSendToClient && newId) {
                try {
                    ...
                    await purchaseInvoiceService.sendInvoiceEmail(emailData);
                    ...
                    if (newId && pendingPaymentDetails.current) {
                        ...
                        const paymentPayload = {
                            ...
                            amount: amountPaid,
                            due_amount: payload.total_amount - amountPaid,
                        ...
                        purchasePaymentService.createPayment(paymentPayload).then((res) => {
                            console.log("Silent purchase payment recorded successfully.");
                            ...
                            if (paymentId) {
                                ...
                                purchasePaymentService.sendPaymentEmail(emailPayload).then(() => {
                                    console.log("Silent purchase payment email sent successfully.");
                                }).catch(e => {
                                    console.error("Silent purchase payment email sending failed:", e);
                                });
                            }
                        }).catch(err => {
                            console.error("Silent purchase payment recording failed:", err);
                        });
                    }
                    ...
                    dispatch(showToast({ message: emailError?.message || "Invoice created, but failed to send email", type: "error" }));
            ...
            if (!error.isHandled) {
                dispatch(showToast({ message: error.message || "Error saving purchase invoice.", type: "error" }));
            }
        } finally {
            setIsSaving(false);
        }
                    ...
                    {step === 1 ? (
                        <div>
                            <div className="flex flex-wrap justify-between items-start mb-6 gap-2 font-poppins">
                                <div>
                                    <h2 className="text-xl md:text-2xl font-semibold text-gray-900">Configure Purchase Invoice</h2>
                                    <p className="text-sm text-gray-500 mt-1">Setup invoice details and items</p>
                                </div>
                                ...
                                <div className="mb-8 font-poppins">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                                        <div className="lg:col-span-3">
                                            <label className="text-xs font-medium text-gray-700 mb-1.5 block">Link Purchase Order</label>
                                            <CustomSelect
                                                value={formData.order_id}
                                                onChange={async (val) => {
                                                    const selectedOrder = purchaseOrders.find(o => o.id == val);
                                                    const updatedFormData = {
                                                        ...formData,
                                                        order_id: val ? Number(val) : null
                                                    };

                                                    if (selectedOrder) {
                                                        const supplier = Array.isArray(selectedOrder.supplier_id) ? selectedOrder.supplier_id[0] : selectedOrder.supplier_id;
                                                        const supplierId = supplier?.id || (typeof selectedOrder.supplier_id !== 'object' ? selectedOrder.supplier_id : "");
                                                        updatedFormData.supplier = supplierId ? String(supplierId) : "";

                                                        updatedFormData.emailConfig = {
                                                            ...updatedFormData.emailConfig,
                                                            to: supplier?.email || (suppliers.find(s => s.id == supplierId)?.email || "")
                                                        };

                                                        // Auto-create invoiceName using purchase order name/number, but user can edit it
                                                        updatedFormData.invoiceName = selectedOrder.order_name || selectedOrder.invoice_name || selectedOrder.order_no || selectedOrder.invoice_number || "";
                                                        ...
                                                        if (Array.isArray(itemsList)) {
                                                            updatedFormData.items = itemsList.map(item => {
                                                                ...
                                                                const resolvedId = sourceType === "raw_material"
                                                                    ? (item.source_id || item.raw_material?.id || "")
                                                                    : sourceType === "service"
                                                                        ? ""
                                                ...
                                                options={purchaseOrders.map(o => ({
                                                    value: o.id,
                                                    label: `${o.invoice_number || o.order_no || ''}${o.invoice_name ? ` - ${o.invoice_name}` : (o.order_name ? ` - ${o.order_name}` : "")}`
                                                }))}
                                                placeholder="Select Purchase Order"
                                                isDisabled={!!formData.returnAgainst || !!editData?.is_auto_created}
                                            />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                                        <div>
                                            <label className="text-xs font-medium text-gray-700 mb-1.5 block">Invoice Number Series</label>
                                            <input
                                                type="text"
                                        ...
                                        <div>
                                            <label className="text-xs font-medium text-gray-700 mb-1.5 block">Invoice Name</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="lg:col-span-3">
                                            ...
                                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center">
                                                <div className="flex-1">
                                                    {(formData.returnAgainst || editData?.is_auto_created || formData.order_id) ? (
                                                        <input
                                                            type="text"
                                                    ...
                                                    {formData.items.map((item, idx) => (
                                                        <tr key={idx} className="border-b border-gray-200">
                                                            ...
                                                            <td className="px-3 py-3 w-36 min-w-[140px]">
                                                                {(editData?.is_auto_created || formData.returnAgainst || formData.order_id) ? (
                                                            ...
                                                            {item.source_type === "service" ? (
                                                                <td colSpan={4} className="px-3 py-3 min-w-[480px]">
                                                                    <input
                                                                        ...
                                                                        className={`w-full px-3 py-2 border border-gray-300 rounded text-sm focus:ring-1 focus:ring-yellow-400 outline-none h-[38px] ${(editData?.is_auto_created || formData.returnAgainst || formData.order_id) ? 'bg-gray-50 cursor-not-allowed text-gray-500' : ''}`}
                                                                        placeholder="Enter service description"
                                                                        readOnly={editData?.is_auto_created || !!formData.returnAgainst || !!formData.order_id}
                                                                    />
                                                                </td>
                                                            ) : (
                                                                <>
                                                                    ...
                                                                    <td className="px-3 py-3 text-center w-28 min-w-[100px]">
                                                                        <div className="flex items-center justify-center gap-1">
                                                                            <button
                                                                                ...
                                                                                className={`w-6 h-6 flex items-center justify-center rounded border border-gray-300 hover:bg-gray-100 text-gray-600 ${(editData?.is_auto_created || formData.order_id) ? 'hidden' : ''}`}
                                                                                disabled={editData?.is_auto_created || !!formData.order_id}
                                                                            >
                                                                                -
                                                                            </button>
                                                                            <input
                                                                                ...
                                                                                className={`w-full max-w-12 px-1 py-1.5 border border-gray-300 rounded text-xs text-center h-[32px] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${(editData?.is_auto_created || formData.order_id) ? 'bg-gray-50 cursor-not-allowed text-gray-500' : ''}`}
                                                                                readOnly={editData?.is_auto_created || !!formData.order_id}
                                                                                placeholder="0"
                                                                            />
                                                                            <button
                                                                    ...
                                                                    <td className="px-3 py-3 text-center w-32 min-w-[120px]">
                                                                        <div className="flex items-center justify-end gap-1 border border-gray-300 rounded px-2 py-1 focus-within:ring-1 focus-within:ring-yellow-400 min-w-[100px] bg-white shadow-sm">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                className={`w-full text-xs outline-none bg-transparent text-right font-bold text-gray-900 ${(editData?.is_auto_created || formData.order_id) ? 'cursor-not-allowed text-gray-500' : ''}`}
                                                                                placeholder="0.00"
                                                                                readOnly={editData?.is_auto_created || !!formData.order_id}
                                                                            />
                                                                        </div>
                                                                ...
                                                                {item.source_type === "service" ? (
                                                                    <div className="flex items-center justify-end gap-1 border border-gray-300 rounded px-2 py-1.5 focus-within:ring-1 focus-within:ring-yellow-400 min-w-[120px] bg-white shadow-sm">
                                                                        ...
                                                                        <input
                                                                            ...
                                                                            value={item.rate}
                                                                            onChange={(e) => handleItemChange(idx, "rate", e.target.value)}
                                                                            className={`w-full text-xs outline-none bg-transparent text-right font-bold text-gray-900 ${(editData?.is_auto_created || formData.returnAgainst || formData.order_id) ? 'cursor-not-allowed text-gray-500' : ''}`}
                                                                            placeholder="0.00"
                                                                            readOnly={editData?.is_auto_created || !!formData.returnAgainst || !!formData.order_id}
                                                                        />
                                                                    </div>
                                                            ...
                                                            <td className="px-3 py-3 text-center w-10 min-w-[40px]">
                                                                {!(editData?.is_auto_created || formData.order_id) && (
                                ...
                                <div className="font-poppins mt-10 md:mt-0">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            {false && (!editData || (editData && editData.return_against)) && !formData.order_id && (
                                                <div>
                                                    <label className="text-xs font-medium text-gray-700 mb-1.5 block">Return Against Invoice</label>
                                                    <CustomSelect
                                                        ...
                                                        onChange={async (val) => {
                                                            ...
                                                            if (selectedInv) {
                                                                const supplier = Array.isArray(selectedInv.supplier_id) ? selectedInv.supplier_id[0] : selectedInv.supplier_id;
                                                                updatedFormData.supplier = supplier?.id || (typeof selectedInv.supplier_id !== 'object' ? selectedInv.supplier_id : "");

                                                                // Pull items from the Invoice directly
                                                                const itemsList = selectedInv.purchase_item || selectedInv.order_item;
                                                                if (Array.isArray(itemsList)) {
                                                                    updatedFormData.items = itemsList.map(item => {
                                                                        let sourceType = item.source_type || "item";
                                                                        if (sourceType === "item") {
                                                                        ...
                                                                        return {
                                                                            ...
                                                                            max_quantity: parseFloat(item.remaining_quantity ?? item.quantity) || 1,
                                                                            rate: parseFloat(item.rate) || 0,
                                                                ...
                                                                if (!formData.invoiceName || formData.invoiceName.startsWith("Return for") || formData.invoiceName.startsWith("Return for Invoice")) {
                                                                    updatedFormData.invoiceName = `Return for ${selectedInv.invoice_name || selectedInv.invoice_number}`;
                                                                }
                                                            }
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start mb-4 gap-4 no-print">
                                <div>
                                    <h2 className="text-xl md:text-2xl font-bold text-gray-900">
                                        {viewOnly ? "View Purchase Invoice" : "Preview & Send"}
                                    </h2>
                                    <p className="text-sm text-gray-500 mt-1">
                                        {isExporting ? "Processing..." : viewOnly ? "View details of your purchase invoice" : "Review and send your purchase invoice"}
                                    </p>
                                </div>
                                ...
                                <div className="bg-white rounded-lg shadow-sm overflow-x-auto md:overflow-x-visible max-w-full md:max-w-4xl mx-auto">
                                    <div className="min-w-[800px] md:min-w-full">
                                        <DocumentPreview
                                            ...
                                            filename={`PurchaseInvoice-${formData.invoiceNumber}`}
                                            payload={{
                                                company: {
                                                    ...
                                                    returnBreakdown: (() => {
                                                        ...
                                                        if (editData?.invoice_summary) {
                                                            ...
                                                            if (selectedInv) {
                                                                ...
                                                                originalTaxTotal = (selectedInv.purchase_item || selectedInv.order_item || []).reduce((sum, item) => {
