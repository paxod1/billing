...
export default function SupplierStatementModal({ isOpen, onClose, supplierId }) {
    ...
    const [filters, setFilters] = useState({
        from_date: "",
        to_date: "",
        type: "PURCHASE" // Default to PURCHASE for suppliers
    });
    ...
    useEffect(() => {
        ...
        // Reset states on opening new supplier
    ...
    const fetchStatement = async (initial = false) => {
        try {
            ...
            dispatch(showToast({ message: "Failed to load supplier statement", type: "error" }));
        } finally {
                ...
                <div className="p-6 bg-white border-b border-gray-200 rounded-t-2xl flex items-center justify-between sticky top-0 z-20 shadow-sm">
                    <div>
                        <h2 className="text-xl md:text-[22px] font-bold text-gray-900">
                            {partyInfo?.name ? `${partyInfo.name} - Statement` : "Supplier Statement"}
                        </h2>
                        {partyInfo && (
                            ...
                            {!activePaymentInvoice && (
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between">
                                        <span className="text-[12px] font-bold text-gray-400 uppercase tracking-wider">Total Billed</span>
                                        ...
                                        <div className="relative flex-1 flex flex-col">
                                            ...
                                            {paginatedInvoices.length > 0 ? (
                                                ...
                                                <EmptyState
                                                    ...
                                                    message="There are no bill records for this supplier statement within the selected filters."
                                                    actionLabel=""
                                                    onActionClick={() => { }}
                                                />
                                            )}
