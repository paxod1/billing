...
import SalesMileageForm from "@/components/sales/SalesMileageForm";
...
export default function SalesMileagePage() {
    return (
...
function SalesMileageContent() {
    ...
    const loadEntries = async (isSilent = false) => {
        ...
        try {
            ...
            const response = await salesMileageService.getMileageEntries({
                ...apiFilters,
                limit: pageSize,
            ...
            dispatch(showToast({ message: "Failed to load mileage entries", type: "error" }));
        } finally {
            setIsLoading(false);
    ...
    // Silently update table when filters or currentPage change
    useEffect(() => {
        if (isMounted.current) {
            loadEntries(true);
        }
    ...
    const handleSaveEntry = async (submission) => {
        ...
        try {
            ...
            await salesMileageService.saveMileageEntry(payload);
            dispatch(showToast({ message: `Mileage Entry ${payload.id ? 'updated' : 'created'} successfully!`, type: "success" }));
            loadEntries(true);
            setIsFormOpen(false);
        } catch (error) {
            console.error("Save Error:", error);
            dispatch(showToast({ message: "Failed to save mileage entry", type: "error" }));
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handlePostClick = async (entry) => {
        ...
        try {
            await salesMileageService.postMileageEntry(entry.id);
            dispatch(showToast({ message: "Mileage Entry posted successfully!", type: "success" }));
        } catch (error) {
            console.error("Post Error:", error);
            // Revert back to original status if failed
            setEntries(prev => prev.map(e => e.id === entry.id ? { ...e, status: entry.status } : e));
            dispatch(showToast({ message: "Failed to post mileage entry", type: "error" }));
        } finally {
    ...
    const handleDeleteEntry = async () => {
        ...
        try {
            setIsDeleting(true);
            await salesMileageService.deleteMileageEntry(entryToDelete.id);
            dispatch(showToast({ message: "Mileage entry deleted successfully", type: "success" }));
            setIsDeleteModalOpen(false);
            setEntryToDelete(null);
            loadEntries(true);
        } catch (error) {
            console.error("Delete Error:", error);
            dispatch(showToast({ message: "Failed to delete mileage entry", type: "error" }));
        } finally {
            setIsDeleting(false);
        }
    };
    ...
    const navbarData = {
        heading: "Sales Mileage Entries",
        subheading: "Track travel distances and manage travel logs",
        from: "common",
    };
        ...
        <div className="flex-1 flex flex-col">
            ...
            {isInitialLoading ? (
                <div className="flex-1 flex flex-col items-center justify-center">
                    <Loader message="Loading Mileage Entries..." />
                </div>
            ) : (
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                                        ...
                                        <button
                                            onClick={() => handleExport({
                                                endpoint: "custom-api/admin/mileage/export/",
                                                dispatch,
                                                setIsExporting,
                                                defaultFileName: "sales_mileage_export.xlsx"
                                            })}
                                        ...
                                        >
                                            Add Mileage Entry <FiPlus size={18} />
                                        </button>
                                    </div>
                                </div>
                            ) : (
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex-1 flex flex-col relative">
                            ...
                            {entries.length > 0 ? (
                                    ...
                                    <div className="min-w-[700px] lg:min-w-0">
                                        <table className="w-full text-left border-collapse">
                                            <thead className="bg-gray-50 border-b border-gray-200">
                                                <tr>
                                                    <th className="px-4 lg:px-6 py-4 text-[14px] lg:text-[15px] font-semibold text-gray-700 whitespace-nowrap rounded-tl-lg">Entry Name</th>
                                                    ...
                                                    <th className="px-4 lg:px-6 py-4 text-[14px] lg:text-[15px] font-semibold text-gray-700 whitespace-nowrap text-right">Distance</th>
                                                ...
                                                {entries.map((entry) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <td className="px-4 lg:px-6 py-4 text-[14px] lg:text-[15px] text-gray-700 whitespace-nowrap text-right font-medium">
                                ...
                                <EmptyState
                                    ...
                                    message={filters.search ? `No entries match your search "${filters.search}".` : "Start by creating your first mileage entry."}
                                    actionLabel="Add Mileage Entry"
                                    onActionClick={() => {
                                        setSelectedEntry(null);
                                        setIsViewOnly(false);
                                        setIsFormOpen(true);
                ...
                <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={() => setIsDeleteModalOpen(false)}>
                    <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 transform transition-all scale-100 animate-in zoom-in-95 duration-200">
                        <div className="text-center">
                            ...
                            <h3 className="text-lg font-medium text-gray-900">Delete Mileage Entry</h3>
