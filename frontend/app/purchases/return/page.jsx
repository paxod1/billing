...
function PurchaseReturnsContent() {
    ...
    const fetchReturns = useCallback(async () => {
        try {
            ...
            const response = await purchaseReturnService.queryReturns(params);
            setReturns(response.data);
            setTotalCount(response.totalCount);
        } catch (error) {
            console.error("Error fetching purchase returns:", error);
            dispatch(showToast({ message: "Failed to load purchase returns", type: "error" }));
        } finally {
            setIsLoading(false);
    ...
    const openEditForm = async (id, isViewOnly = false) => {
        try {
            setOpenMenuId(null);
            setLoadingEditId(id);
            const result = await purchaseReturnService.getReturnByIdDeep(id);
            if (result.success) {
                setEditData(result.data);
                setViewOnly(isViewOnly);
                setIsFormOpen(true);
            } else {
                dispatch(showToast({ message: "Failed to load purchase return details", type: "error" }));
            }
        } catch (error) {
    ...
    const handleDeleteConfirm = async (id) => {
        try {
            ...
            dispatch(showToast({ message: "Purchase Return deleted successfully", type: "success" }));
            fetchReturns();
    ...
    const handleSendToSupplierDirect = async (id) => {
        ...
        try {
            ...
            if (!ret) return;
            const supplier = ret.supplier_id;
            const emailData = {
            ...
            const response = await purchaseReturnService.sendReturnEmail(emailData);
            if (response && response.success === false) {
                throw new Error(response.message || "Failed to send email");
            }
            dispatch(showToast({ message: "Purchase Return sent successfully!", type: "success" }));
            setTimeout(() => {
                setSendingReturns(prev => {
                    const newSending = { ...prev };
            ...
            console.error("Error sending purchase return email:", error);
    ...
    const navbarData = {
        ...
        subheading: "Manage and track your purchase returns",
        from: "purchases",
    };
            ...
            <main className="flex-1 flex flex-col py-6 md:py-8">
                {isLoading ? (
                    <div className="flex-1 flex flex-col items-center justify-center">
                        <Loader message="Loading Purchase Returns..." />
                    </div>
                ) : (
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                                        ...
                                        <button
                                            onClick={() => handleExport({
                                                ...
                                                payload: { find: { ...filters.search ? { invoice_number: { "$like": `%${filters.search}%` } } : {}, return_against: { "$gt": 0 } } },
                                                dispatch,
                                                setIsExporting,
                                                defaultFileName: "purchase_returns_export.xlsx"
                                            })}
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex-1 flex flex-col relative">
                            ...
                            {returns.length > 0 ? (
                                        ...
                                        <tbody className="divide-y divide-gray-100 font-poppins">
                                            {returns.map((ret) => (
                                                <tr key={ret.id} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="px-4 lg:px-6 py-4 text-[14px] lg:text-[15px] text-gray-700 whitespace-nowrap">
                                                        {(Array.isArray(ret.supplier_id) ? ret.supplier_id[0]?.name : ret.supplier_id?.name) || "N/A"}
                                                    </td>
                                ...
                                <EmptyState
                                    ...
                                    message={searchInput ? `No returns match your search "${searchInput}".` : "Start by creating your first purchase return."}
